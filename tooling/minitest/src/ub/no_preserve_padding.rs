...
fn no_preserve_padding() {
    ...
    dump_program(p);
    assert_ub(p, "load at type PlaceType { ty: Int(IntType { signed: Unsigned, size: Size { raw: Int(Small(1)) } }), align: Align(Int(Small(1))) } but the data in memory violates the validity invariant");
}
